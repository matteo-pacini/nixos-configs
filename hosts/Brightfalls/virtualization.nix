...
}:
{
  ...
  boot.extraModulePackages = lib.mkIf (!isVM) [ config.boot.kernelPackages.vendor-reset ];
  boot.kernelModules = lib.mkIf (!isVM) [ "vendor-reset" ];

  # Kernel params for IOMMU (required for GPU passthrough)
  boot.kernelParams = lib.mkIf (!isVM) [
  ...
  systemd.tmpfiles.rules = lib.mkIf (!isVM) [
  ...
  systemd.services.libvirt-nosleep = lib.mkIf (!isVM) {
