# Auto-generated by compose2nix, now hand-maintained.
# DB passwords come from agenix (nexus/n8n-env) via environmentFiles —
# do NOT regenerate this file from docker-compose.yml without re-applying that.

{
  pkgs,
  lib,
  config,
  ...
}:

{
  # Runtime
  virtualisation.podman = {
    enable = true;
    autoPrune.enable = true;
    dockerCompat = true;
  };

  # Enable container name DNS for all Podman networks.
  networking.firewall.interfaces =
    let
      matchAll = if !config.networking.nftables.enable then "podman+" else "podman*";
    in
    {
      "${matchAll}".allowedUDPPorts = [ 53 ];
    };

  virtualisation.oci-containers.backend = "podman";

  # Containers
  virtualisation.oci-containers.containers."nexus-n8n-n8n" = {
    image = "docker.n8n.io/n8nio/n8n:2.15.0";
    environment = {
      "DB_POSTGRESDB_DATABASE" = "n8n";
      "DB_POSTGRESDB_HOST" = "postgres";
      "DB_POSTGRESDB_PORT" = "5432";
      "DB_POSTGRESDB_USER" = "user";
      "DB_TYPE" = "postgresdb";
      "GENERIC_TIMEZONE" = "Europe/London";
      "N8N_COMMUNITY_PACKAGES_ALLOW_TOOL_USAGE" = "true";
      "N8N_HOST" = "n8n.matteopacini.me";
      "N8N_PORT" = "5678";
      "N8N_PROTOCOL" = "http";
      "N8N_PROXY_HOPS" = "1";
      "N8N_SECURE_COOKIE" = "false";
      "NODE_ENV" = "production";
      "NODE_FUNCTION_ALLOW_BUILTIN" = "*";
      "NODE_FUNCTION_ALLOW_EXTERNAL" = "*";
      "TZ" = "Europe/London";
      "WEBHOOK_URL" = "https://n8n.matteopacini.me";
    };
    environmentFiles = [ config.age.secrets."nexus/n8n-env".path ];
    volumes = [
      "/var/lib/n8n:/home/node/.n8n:rw"
    ];
    ports = [
      "0.0.0.0:5678:5678/tcp"
    ];
    dependsOn = [
      "nexus-n8n-postgres"
    ];
    log-driver = "journald";
    extraOptions = [
      "--add-host=gateway.matteopacini.me:192.168.10.14"
      "--add-host=home.matteopacini.me:192.168.10.14"
      "--add-host=jellyfin.matteopacini.me:192.168.10.14"
      "--add-host=n8n.matteopacini.me:192.168.10.14"
      "--network-alias=n8n"
      "--network=nexus-n8n_default"
    ];
  };
  systemd.services."podman-nexus-n8n-n8n" = {
    serviceConfig = {
      Restart = lib.mkOverride 90 "always";
    };
    after = [
      "podman-network-nexus-n8n_default.service"
    ];
    requires = [
      "podman-network-nexus-n8n_default.service"
    ];
    partOf = [
      "podman-compose-nexus-n8n-root.target"
    ];
    wantedBy = [
      "podman-compose-nexus-n8n-root.target"
    ];
  };
  virtualisation.oci-containers.containers."nexus-n8n-postgres" = {
    image = "postgres:16.13";
    environment = {
      "POSTGRES_DB" = "n8n";
      "POSTGRES_USER" = "user";
    };
    environmentFiles = [ config.age.secrets."nexus/n8n-env".path ];
    volumes = [
      "/var/lib/postgresql_n8n:/var/lib/postgresql/data:rw"
    ];
    log-driver = "journald";
    extraOptions = [
      "--health-cmd=pg_isready -h localhost -U user -d n8n"
      "--health-interval=5s"
      "--health-retries=10"
      "--health-timeout=5s"
      "--network-alias=postgres"
      "--network=nexus-n8n_default"
    ];
  };
  systemd.services."podman-nexus-n8n-postgres" = {
    serviceConfig = {
      Restart = lib.mkOverride 90 "always";
    };
    after = [
      "podman-network-nexus-n8n_default.service"
    ];
    requires = [
      "podman-network-nexus-n8n_default.service"
    ];
    partOf = [
      "podman-compose-nexus-n8n-root.target"
    ];
    wantedBy = [
      "podman-compose-nexus-n8n-root.target"
    ];
  };

  # Networks
  systemd.services."podman-network-nexus-n8n_default" = {
    path = [ pkgs.podman ];
    serviceConfig = {
      Type = "oneshot";
      RemainAfterExit = true;
      ExecStop = "podman network rm -f nexus-n8n_default";
    };
    script = ''
      podman network inspect nexus-n8n_default || podman network create nexus-n8n_default
    '';
    partOf = [ "podman-compose-nexus-n8n-root.target" ];
    wantedBy = [ "podman-compose-nexus-n8n-root.target" ];
  };

  # Root service
  # When started, this will automatically create all resources and start
  # the containers. When stopped, this will teardown all resources.
  systemd.targets."podman-compose-nexus-n8n-root" = {
    unitConfig = {
      Description = "Root target generated by compose2nix.";
    };
    wantedBy = [ "multi-user.target" ];
  };
}
