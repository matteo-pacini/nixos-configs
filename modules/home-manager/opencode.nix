...
let
  ...
  opencodeBaseConfig = {
    ...
    # https://openrouter.ai/moonshotai/kimi-k2.6  (~$0.74 in / $3.49 out per Mtok, 256k ctx)
    ...
    agent = {
      ...
      #   $0.435 in / $0.87 out per Mtok with 1M context — 4.0x cheaper
      #   on output than Kimi K2.6 ($0.74 / $3.49, 256K).
      #   PROMO ENDS 2026-05-31 15:59 UTC → list price rises to $1.74 / $3.48
      #   (essentially matches Kimi on output; remains useful when 1M
      #   context or a different reasoner profile is the deciding factor).
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    ++ lib.optional cfg.localLlama.enable cfg.localLlama.llamaCppPackage;
    home.file.".config/opencode/themes/dracula.json".source =
      "${inputs.opencode-dracula-theme}/dracula.json";
    ...
    home.file.".config/opencode/opencode.json".text = builtins.toJSON (
      lib.recursiveUpdate opencodeBaseConfig localLlamaProvider
    );
  };
}
