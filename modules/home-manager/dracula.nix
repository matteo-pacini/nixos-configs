...
in {
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf
      (cfg.vscode.enable && config.programs.vscode.enable)
      {
    ...
    (lib.mkIf
      (cfg.xcode.enable && config.programs.xcodes.enable)
      {
    ...
    (lib.mkIf
      (cfg.fzf.enable && config.programs.fzf.enable)
      {
        programs.fzf = {
          defaultOptions = [
    ...
    (lib.mkIf
      (cfg.bat.enable && config.programs.bat.enable)
      {
        programs.bat = {
          themes = {
            dracula = {
